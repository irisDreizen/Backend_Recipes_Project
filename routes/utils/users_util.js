...
async function getMyRecipes_full(username, recipeID) {
    const myRecipes = await DButils.execQuery(`SELECT * FROM dbo.personalRecipes WHERE username='${username}' AND recipeId='${recipeID}'`);
    const jsonWithoutInstructions = await extractData_full(myRecipes, username);
